...
def leastSq(t, yE, v, V0, M0, A0, par):
    ...
    initial value
    """            
    return least_squares(residuals, par, method='trf')
...
if __name__ == "__main__":
    ...
    plt.legend()
    plt.savefig("result/fitting_12.png")
    plt.show()
    ...
    plt.xticks(range(len(pp)),labels=["n", "Tmix", "Titc", "koff", "Kd", "H", "Pdil"])
    plt.yticks(range(len(pp)), labels=["n", "Tmix", "Titc", "koff", "Kd", "H", "Pdil"])    
    plt.colorbar()
    plt.savefig("result/pcor_12.png")
    plt.close()


    rnames = ["n", "Tmix", "Titc", "koff", "Kd", "H", "Pdil"]

    df = pd.DataFrame({'coef':pp, 'se':np.sqrt(np.diag(pcov))}, index=rnames)
    df = df.join(pd.DataFrame(pcor,columns=rnames,index=rnames))
    df.to_csv("result/parameters_12.csv")   
